...
| SQLAlchemy, Schema, ORM, Database (Postgres) related tasks | `sqlalchemy-postgres` |
...
**API Stack**: Python 3.12+, FastAPI, SQLAlchemy 2, Pydantic, PostgreSQL — tooling: uv (deps/venv), ruff (lint+format), pytest (all shimmed through `pnpm lint/test/format` via turbo)
