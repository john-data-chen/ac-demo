...
async def test_board_not_found(client, user_ids):
    ...
    async with client as c:
        ...
        r1 = await c.get(f"/boards/{uuid.uuid4()}", cookies={"jwt": token})
        assert r1.status_code == 404
        r2 = await c.get("/boards/invalid-uuid", cookies={"jwt": token})
...
async def test_board_update_access_and_fields(client, user_ids, test_engine):
    ...
    async with client as c:
        ...
        r1 = await c.patch(f"/boards/{board_id}", json={"title": "T"}, cookies={"jwt": token2})
        assert r1.status_code == 404
...
async def test_board_members(client, user_ids, test_engine):
    ...
    async with client as c:
        ...
        r_add_fail = await c.post(
            f"/boards/{board_id}/members/{uuid.uuid4()}", cookies={"jwt": token1}
        )
        assert r_add_fail.status_code == 404

        # Add member 404 (invalid user uuid)
        r_add_fail2 = await c.post(f"/boards/{board_id}/members/invalid", cookies={"jwt": token1})
        assert r_add_fail2.status_code == 404

        # Add member 403 (non-owner)
        r_add_fail3 = await c.post(f"/boards/{board_id}/members/{id2}", cookies={"jwt": token2})
        assert r_add_fail3.status_code == 403

        # Add member success
        r_add = await c.post(f"/boards/{board_id}/members/{id2}", cookies={"jwt": token1})
        assert r_add.status_code == 200

        # Remove member 404 invalid uuid
        r_rem_fail = await c.delete(f"/boards/{board_id}/members/invalid", cookies={"jwt": token1})
        assert r_rem_fail.status_code == 404

        # Remove member 403 non-owner
        r_rem_fail2 = await c.delete(f"/boards/{board_id}/members/{id2}", cookies={"jwt": token2})
        assert r_rem_fail2.status_code == 403

        # Remove member success
        r_rem = await c.delete(f"/boards/{board_id}/members/{id2}", cookies={"jwt": token1})
...
async def test_project_errors(client, user_ids, test_engine):
    ...
    async with client as c:
        ...
        r = await c.post(
            "/projects", json={"title": "P", "boardId": "invalid"}, cookies={"jwt": token1}
        )
        assert r.status_code == 400

        # board not found in create
        r = await c.post(
            "/projects", json={"title": "P", "boardId": str(uuid.uuid4())}, cookies={"jwt": token1}
        )
        assert r.status_code == 400
        ...
        r = await c.post(
            "/projects", json={"title": "P2", "boardId": board_id}, cookies={"jwt": token1}
        )
        assert r.status_code == 201

        # get projects by board invalid id
        r = await c.get("/projects?boardId=invalid", cookies={"jwt": token1})
        assert r.status_code == 400

        # update project non-owner tries to update title
        r = await c.patch(f"/projects/{p1_id}", json={"title": "X"}, cookies={"jwt": token2})
        assert r.status_code == 400
        ...
        # project not found
        r = await c.patch(f"/projects/{uuid.uuid4()}", json={"title": "T"}, cookies={"jwt": token1})
        assert r.status_code == 404
        r = await c.patch("/projects/invalid", json={"title": "T"}, cookies={"jwt": token1})
        assert r.status_code == 404

        # delete project
        r = await c.delete(f"/projects/{p1_id}", cookies={"jwt": token1})
        assert r.status_code == 200
...
async def test_task_errors_and_moves(client, user_ids, test_engine):
    ...
    async with client as c:
        ...
        # invalid create
        r = await c.post(
            "/tasks", json={"title": "T", "board": "inv", "project": "inv"}, cookies={"jwt": token1}
        )
        assert r.status_code == 400

        # update task forbidden
        r = await c.patch(f"/tasks/{t1}", json={"title": "X"}, cookies={"jwt": token2})
        ...
        # find tasks query
        r = await c.get(f"/tasks?projectId={p1_id}&assigneeId={id1}", cookies={"jwt": token1})
        assert r.status_code == 200

        # find tasks query invalid
        r = await c.get("/tasks?projectId=inv&assigneeId=inv", cookies={"jwt": token1})
        assert r.status_code == 200
        ...
        # task not found
        r = await c.get(f"/tasks/{uuid.uuid4()}", cookies={"jwt": token1})
        assert r.status_code == 404
        r = await c.get("/tasks/invalid", cookies={"jwt": token1})
        assert r.status_code == 404

        # delete task
        r = await c.delete(f"/tasks/{t1}", cookies={"jwt": token1})
        assert r.status_code == 200
