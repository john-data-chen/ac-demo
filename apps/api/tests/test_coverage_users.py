...
# Shared fixtures from test_coverage.py
...
async def test_users_router(client, user_ids):
    ...
    async with client as c:
        ...
        # Test find_all
        r_all = await c.get("/users", cookies={"jwt": token})
        assert r_all.status_code == 200
        assert len(r_all.json()["users"]) >= 2

        # Test search
        r_search = await c.get("/users/search?username=Cov", cookies={"jwt": token})
...
async def test_auth_exceptions(client, user_ids, test_engine):
    ...
    async with client as c:
        ...
        # 401 Invalid token
        r_inv = await c.get("/users", cookies={"jwt": "invalid-token-string"})
        ...
        r_exp = await c.get("/users", cookies={"jwt": expired_token})
        ...
        r_notfound = await c.get("/users", cookies={"jwt": fake_token})
